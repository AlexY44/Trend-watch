...
def fetch(code, period, interval):
    try:
        ...
        df.loc[bsc>=3,'sig']=1; df.loc[ssc>=3,'sig']=-1
        df['trend']='Range'
...
def run_backtest(df, cost=0.001):
    ...
    pf=abs(sum(t['ret'] for t in wins)/sum(t['ret'] for t in loss)) if loss else float('inf')
...
for k,v in [('result',None),('code',''),('period','2y'),('interval','1d'),
...
if fbtn and typed.strip():
    code=typed.strip().upper(); fetch.clear()
...
tab1,tab2,tab3,tab4=st.tabs(['Chart','Backtest','Signals','Data'])
...
with tab2:
    ...
    st.caption('Rule: Buy signal next bar -> hold -> Sell signal next bar -> repeat')
    cost_pct=st.slider('Transaction cost per side (%)',0.0,1.0,0.1,0.05)
    bt=run_backtest(df,cost=cost_pct/100)
    if bt is None:
        st.warning('Not enough trades. Try a longer period (2Y or more recommended).')
    else:
        s=bt['stats']
        diff=s['sr']-s['bh']
        ...
        with st.spinner('Drawing equity curve...'):
            fig2=make_bt_chart(bt,f"{code} Backtest  {PL[per]}  cost={cost_pct}%/side")
