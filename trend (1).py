"""
Trend Signal PRO  v5.1
- Streamlit Cloud 対応（requirements.txt と合わせて使用）
- ロード不能バグ修正:
    bsc.where() の整数型エラー対策 → float変換
    st.stop() 後のコード位置修正
    DEFAULT_SIG の辞書破壊バグ根絶
- WF最適化: 729通り（重みは固定、閾値のみ）→ 約15〜60秒で完了
"""
...
# シグナルパラメータ（辞書破壊されないよう、関数内でコピーして使う）
DEFAULT_SIG = {
    'adx_th':          20,
    'rsi_buy_th':      45,
    'rsi_sell_th':     55,
...
def compute_indicators(df):
    ...
    df['EMA20_slope'] = (df['EMA20'] / df['EMA20'].shift(5).replace(0, np.nan) - 1) * 100
    df['EMA50_slope'] = (df['EMA50'] / df['EMA50'].shift(10).replace(0, np.nan) - 1) * 100
...
def compute_signals(df, p=None):
    """
    p は DEFAULT_SIG のキーのみを持つ辞書（atr_multは含まない）
    元の辞書は一切変更しない
    """
    # DEFAULT_SIG をコピーして上書き
    ...
    ema_up   = (s['EMA20'] > s['EMA50']) & (s['EMA20_slope'].fillna(0) > 0)
    ema_down = (s['EMA20'] < s['EMA50']) & (s['EMA20_slope'].fillna(0) < 0)
    ...
    # ---- 買いスコア（float型で作成 → where()エラー回避）----
    bsc = pd.Series(0.0, index=s.index)
    bsc += (macd_xu & (s['MACD'] > 0)).astype(float) * 2.0   # MACDゼロライン上クロス
    bsc += (macd_xu & (s['MACD'] <= 0)).astype(float) * 1.0  # MACDゼロライン下クロス
    bsc += (s['RSI_s'] > cfg['rsi_buy_th']).astype(float)
    bsc += (sk_xu & (s['SK'] < cfg['stoch_buy_th'])).astype(float)
    bsc += (s['BB_pct'] < 0.4).astype(float)
    # トレンドフィルター: 上昇トレンド+ADX+DI条件が揃わない場合スコアを半減
    trend_ok = ema_up & adx_ok & di_up
    bsc = bsc.where(trend_ok, bsc * 0.5)   # // 2 ではなく * 0.5 で整数型問題を回避

    # ---- 売りスコア ----
    ssc = pd.Series(0.0, index=s.index)
    ssc += (ema_down & adx_ok & di_down).astype(float) * 2.0  # トレンド崩壊
    ssc += (macd_xd & (s['MACD'] < 0)).astype(float) * 2.0
    ssc += (macd_xd & (s['MACD'] >= 0)).astype(float) * 1.0
    ssc += (s['RSI_s'] < cfg['rsi_sell_th']).astype(float)
    ssc += (sk_xd & (s['SK'] > cfg['stoch_sell_th'])).astype(float)
    ssc += (s['BB_pct'] > 0.7).astype(float)

    df = df.copy()
    df['bsc'] = bsc
    df['ssc'] = ssc
    df['sig'] = 0
    df.loc[bsc >= float(cfg['buy_th']),  'sig'] =  1
    df.loc[ssc >= float(cfg['sell_th']), 'sig'] = -1
...
def score_params(df, p, cost):
    ...
    if bt is None:
        return -9999.0
    s = bt['stats']
    if s['n'] < 5 or s['mdd'] < -55:
        return -9999.0
    wr_pen = max(0.0, (40.0 - s['wr']) * 0.05)
    dd_pen = max(0.0, (-s['mdd'] - 30.0) * 0.2)
...
def walk_forward_optimize(code, period, interval, n_splits=4, cost=0.001):
    ...
    fold_results = []; combined_trades = []; combined_eq = []
    current_eq = 1.0

    progress = st.progress(0, f'WF最適化中... ({n_combos}通り × {n_splits} folds)')
    ...
    full_stats = {
        ...
        'sharpe': float(pc.mean() / pc.std() * np.sqrt(252)) if pc.std() > 0 else 0.0,
    }
...
def make_chart(df, title, mobile=False):
    ...
    if bi:
        ax0.scatter(bi, plot_df.loc[bi, 'Low'] * 0.995,
                    marker='^', color=C['buy'],  s=65, zorder=6, label='Buy')
    if si:
        ax0.scatter(si, plot_df.loc[si, 'High'] * 1.005,
    ...
    try:
        labs = [str(plot_df.iloc[i][dcol])[:10] for i in tpos]
    except Exception:
...
if res is None:
    ...
    with tab1:
        last = df.iloc[-1]
        sig_label = {1: '🟢 BUY', -1: '🔴 SELL', 0: '⚪ NEUTRAL'}.get(
            int(last['sig']), '⚪ NEUTRAL')
    ...
    with tab3:
        ...
        **Expanding Window Walk-Forward Optimization**
        - グリッド: **{n_combos_disp}通り**（閾値のみ最適化、重みは固定）
        - Train累積拡大 → 後半Foldほど安定した選択
        ...
        if wf:
            st.success(f"✅ WF完了（{wf['n_combos']}通り × {n_splits} folds）")
            s = wf['full_bt']['stats']
            c1, c2, c3, c4, c5 = st.columns(5)
