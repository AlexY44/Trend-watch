...
# PARAM_GRID: stoch_buy_th / stoch_sell_th を含む完全版
PARAM_GRID = {
    'w_trend':       [1, 2],
    'w_macd':        [1, 2, 3],
    'rsi_buy_th':    [25, 30, 35],
...
def compute_signals(df, p):
    # DEFAULT_PARAMSをベースにマージ → キー不足によるKeyErrorを防ぐ
    p = {**DEFAULT_PARAMS, **p}
    s = df
...
def run_backtest(df, cost=0.001, initial_equity=1.0):
    ...
    # ★ Buy & Hold: initial_equityと同スケールで正規化
    ...
    return {
        'trades':    trades,
        'equity':    eq_s,
        'bh_series': bh_series,   # ★ Buy & Hold曲線
        'drawdown':  dd,
        'stats': {
            'n': n, 'wr': wr, 'aw': aw, 'al': al, 'pf': pf,
            'sr': (eq / initial_equity - 1) * 100,
            'bh': bh_pct, 'mdd': mdd, 'cagr': cagr, 'sharpe': sharpe,
        },
    }
...
def score_params(df, p, cost):
    """
    最適化スコア関数（改善版）
    - シャープレシオ重視でリスク調整後リターンを評価
    - トレード数が少ない場合はペナルティ（過学習防止）
    """
    full_p = {**DEFAULT_PARAMS, **p}
    df2 = compute_signals(df, full_p)
    bt  = run_backtest(df2, cost)
    if bt is None or bt['stats']['n'] < 3: return -999
    s = bt['stats']
    trade_penalty = max(0, (10 - s['n']) * 0.5)
    return s['sharpe'] * 5 + s['sr'] * 0.3 - max(0, -s['mdd']) * 0.8 - trade_penalty

# =========================================================
# --- ウォークフォワード最適化
# =========================================================
# 【修正】@st.cache_data を除去
# → キャッシュが残ることで銘柄切り替え後も前の結果が返り続ける問題を解消
...
def make_chart(df, title, mobile=False):
    ...
    ax3.fill_between(xs, plot_df['RSI'], 70, where=plot_df['RSI'] >= 70,
                     color=C['sell'], alpha=0.2)
    ax3.fill_between(xs, plot_df['RSI'], 30, where=plot_df['RSI'] <= 30,
                     color=C['buy'],  alpha=0.2)
    ...
    ax4.set_xticklabels(date_labels, rotation=30, ha='right', fontsize=6, color=C['sub'])
...
def make_bt_chart(bt, title, bt2=None, label='Strategy'):
    """
    バックテスト結果チャート
    ★ Buy & Hold ラインを常に表示（黄色の点線）
    bt2 = WFタブのDefault Strategy比較用（任意）
    """
    eq = bt['equity']
    dd = bt['drawdown']
    bh = bt.get('bh_series')
...
# セッションステート初期化
for key, val in [('active_params', DEFAULT_PARAMS), ('wf_result', None),
...
with st.sidebar:
    ...
    if st.button("🔍 Analyze", type="primary", use_container_width=True):
        # 【修正】銘柄が変わった場合は wf_result も含め完全リセット
...
if res is None:
    ...
    with tab3:
        ...
        if wf:
            ...
            # ★ Buy & Hold + Default Strategy + WF Strategy の3本比較
